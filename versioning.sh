...
then
    sed -i "s/^BUILD_VERSION\=.*/BUILD_VERSION=0/" /var/jenkins_home/envstore/frontend.env
...
sed -i "s/^BUILD_DATE\=.*/BUILD_DATE=$DATE_NOW/" /var/jenkins_home/envstore/frontend.env
sed -i "s/^BUILD_VERSION\=.*/BUILD_VERSION=$NEXT_VERSION/" /var/jenkins_home/envstore/frontend.env
