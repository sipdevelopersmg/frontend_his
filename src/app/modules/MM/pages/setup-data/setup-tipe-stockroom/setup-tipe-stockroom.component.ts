...
import { SetupTipeStockroomModel } from '../../../models/setup-data/setup-tipe-stockroom/SetupTipeStockroom';
import { SetupTipeStockroomService } from '../../../services/setup-data/setup-tipe-stockroom/setup-tipe-stockroom.service';

@Component({
  selector: 'app-setup-tipe-stockroom',
  templateUrl: './setup-tipe-stockroom.component.html',
  styleUrls: ['./setup-tipe-stockroom.component.css']
})
export class SetupTipeStockroomComponent implements OnInit {
    ...
    */
    ...
    constructor(
      ...
      private setupTipeStockroomService: SetupTipeStockroomService
    ) {
      this.FormInputData = this.formBuilder.group({
        id_tipe_stockroom: ['', [Validators.required]],
        tipe_stockroom: ['', [Validators.required]],
    ...
    ResetFrom(): void {
      this.FormInputData.reset();
      this.tipe_stockroom.setValue('');
      this.keterangan.setValue('');
    }
  
    /** Method Untuk Mereload Data Grid */
    GetAllData(): void {
      this.setupTipeStockroomService.onGetAll()
        .subscribe((result) => {
          this.GridDatasource = result.data;
        });
    }
  
    AddDataTipeStockroom(): void {
      console.log('Add');
    }
    ...
    SaveAndNew(): void {
      ...
      if (this.inputFieldState=='normal') {
        this.setupTipeStockroomService.onPostSave(Data)
          .subscribe((result: SetupTipeStockroomModel) => {
        ...
        this.setupTipeStockroomService.onPutEdit(Data)
          .subscribe((result: SetupTipeStockroomModel) => {
    ...
    get tipe_stockroom(): AbstractControl { return this.FormInputData.get('tipe_stockroom'); }
