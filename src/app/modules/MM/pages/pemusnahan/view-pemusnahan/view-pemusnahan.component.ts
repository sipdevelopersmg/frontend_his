...
@Component({
  selector: 'app-view-pemusnahan',
  templateUrl: './view-pemusnahan.component.html',
  styleUrls: ['./view-pemusnahan.component.css']
})
export class ViewPemusnahanComponent implements OnInit {
  ...
  onLoadDetailData(id){
      this.pemusnahanService.onGetById(id).subscribe((result)=>{
          ...
          this.pemusnahanService.setDetail(id);
      });
  }
  ...
  onValidation(): void {
    this.pemusnahanService.Validation(this.id).subscribe((result)=>{
  ...
  onCalceled(): void {
    ...
    this.pemusnahanService.Batal(this.id,reason_canceled).subscribe((result)=>{
