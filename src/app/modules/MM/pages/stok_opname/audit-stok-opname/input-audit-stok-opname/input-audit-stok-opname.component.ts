...
import { EditSettingsModel, IEditCell, GridComponent, GridModel, AddEventArgs } from '@syncfusion/ej2-angular-grids';
...
@Component({
  selector: 'app-input-audit-stok-opname',
  templateUrl: './input-audit-stok-opname.component.html',
  styleUrls: ['./input-audit-stok-opname.component.css']
})

export class InputAuditStokOpnameComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.itemsParams = {
        ...
        write: () => {
            this.auditStokOpnameService.onGetBatchED({
  ...
  handleChangeStockroom(args): void{
    console.log(args)
    this.auditStokOpnameService.onGetRak({
  ...
  handleClickGetBarang():void{
    ...
    console.log(data);
    this.auditStokOpnameService.onGetBarang(data).subscribe((result)=>{
  ...
  insert() {
    ...
    this.auditStokOpnameService.Insert(Data).subscribe((result)=>{
