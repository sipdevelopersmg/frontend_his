...
import { SetupOutletModel } from '../../../models/setup-data/SetupOutlet';
...
import { SetupTipeOutletService } from '../../../services/setup-data/setup-tipe-outlet/setup-tipe-outlet.service';

@Component({
  selector: 'app-setup-outlet',
  templateUrl: './setup-outlet.component.html',
  styleUrls: ['./setup-outlet.component.css']
})
export class SetupOutletComponent implements OnInit {
  MaritalTipeOutletDropdownField: object = { text: 'tipe_outlet', value: 'id_tipe_outlet' };
  ...
  */
  ...
  constructor(
    ...
    private setupOutletService: SetupOutletService,
    public setupStockroomService: SetupStockroomService,
    public setupTipeOutletService: SetupTipeOutletService,
  ) {
    this.FormInputData = this.formBuilder.group({
      id_outlet: [0, []],
      id_stockroom: [0, [Validators.required]],
      kode_outlet: ['', [Validators.required]],
      nama_outlet: ['', [Validators.required]],
      id_tipe_outlet: [0, [Validators.required]],
      is_active: [false, []]
    });
  }
  ...
  SaveAndNew(): void {
    ...
    if (this.inputFieldState=='normal') {
      this.setupOutletService.onPostSave(Data)
        .subscribe((result: SetupOutletModel) => {
  ...
  */
  SetActive(is_active: boolean, id_outlet: number): void {
    let data = {
      id_outlet: id_outlet
    }
  ...
  get id_stockroom(): AbstractControl { return this.FormInputData.get('id_stockroom'); }
  get kode_outlet(): AbstractControl { return this.FormInputData.get('kode_outlet'); }
  get nama_outlet(): AbstractControl { return this.FormInputData.get('nama_outlet'); }
  get id_tipe_outlet(): AbstractControl { return this.FormInputData.get('id_tipe_outlet'); }
