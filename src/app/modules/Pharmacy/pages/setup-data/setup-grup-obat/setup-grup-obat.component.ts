...
import { SetupGrupObatModel } from '../../../models/setup-data/SetupGrupObat';
import { SetupGrupObatService } from '../../../services/setup-data/setup-grup-obat/setup-grup-obat.service';

@Component({
  selector: 'app-setup-grup-obat',
  templateUrl: './setup-grup-obat.component.html',
  styleUrls: ['./setup-grup-obat.component.css']
})
export class SetupGrupObatComponent implements OnInit {
  ...
  */
  ...
  constructor(
    ...
    private setupGrupObatService: SetupGrupObatService
  ) {
    this.FormInputData = this.formBuilder.group({
      id_grup_obat: ['', []],
      kode_grup_obat: ['', [Validators.required]],
      nama_grup_obat: ['', [Validators.required]],
      is_active: [false, []]
    });
  }
  ...
  SaveAndNew(): void {
    ...
    if (this.inputFieldState=='normal') {
      this.setupGrupObatService.onPostSave(Data)
        .subscribe((result: SetupGrupObatModel) => {
  ...
  */
  SetActive(is_active: boolean, id_grup_obat: number): void {
    let data = {
  ...
  get kode_grup_obat(): AbstractControl { return this.FormInputData.get('kode_grup_obat'); }
  get nama_grup_obat(): AbstractControl { return this.FormInputData.get('nama_grup_obat'); }
