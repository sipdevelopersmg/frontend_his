...
import { SetupLabelPemakaianObatModel } from '../../../models/setup-data/SetupLabelPemakaianObat';
...
@Component({
  selector: 'app-setup-label-pemakaian-obat',
  templateUrl: './setup-label-pemakaian-obat.component.html',
  styleUrls: ['./setup-label-pemakaian-obat.component.css']
})
export class SetupLabelPemakaianObatComponent implements OnInit {
   ...
   */
   ...
   constructor(
     ...
     private setupLabelPemakaianObatService: SetupLabelPemakaianObatService
   ) {
     this.FormInputData = this.formBuilder.group({
       id_label_pemakaian_obat: [0, []],
       kode_label_pemakaian_obat: ['', [Validators.required]],
       nama_label_pemakaian_obat: ['', [Validators.required]],
       berapa_kali_per_hari: [0, [Validators.required]],
       is_active: [false, []]
     });
   }
   ...
   AddDataLabelPemakaianObat(): void {
     console.log('Add');
   }
   ...
   SaveAndNew(): void {
     ...
     if (this.inputFieldState=='normal') {
       this.setupLabelPemakaianObatService.onPostSave(Data)
         .subscribe((result: SetupLabelPemakaianObatModel) => {
   ...
   */
   SetActive(is_active: boolean, id_label_pemakaian_obat: number): void {
     let data = {
   ...
   get kode_label_pemakaian_obat(): AbstractControl { return this.FormInputData.get('kode_label_pemakaian_obat'); }
   get nama_label_pemakaian_obat(): AbstractControl { return this.FormInputData.get('nama_label_pemakaian_obat'); }
   get berapa_kali_per_hari(): AbstractControl { return this.FormInputData.get('berapa_kali_per_hari'); }
