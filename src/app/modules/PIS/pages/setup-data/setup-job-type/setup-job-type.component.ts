...
import { JobTypeModel } from '../../../models/setup-data/setup-job-type.model';
import { SetupJobTypeService } from '../../../services/setup-data/setup-job-type/setup-job-type.service';
import * as Config from './json/setup-job-type.config.json';

@Component({
    selector: 'app-setup-job-type',
    templateUrl: './setup-job-type.component.html',
    styleUrls: ['./setup-job-type.component.css']
})
export class SetupJobTypeComponent implements OnInit {
    ...
    */
    ...
    ResetFrom(): void {
        ...
        this.job_type.setValue('');
    }

    /** Method Untuk Mereload Data Grid */
    GetAllData(): void {
        this.setupJobTypeService.onGetAll()
            .subscribe((result) => {
    ...
    SetFrom(Data: JobTypeModel): void {
        this.FormInputData.reset();
        this.FormInputData.setValue(Data);

        console.log(this.FormInputData.value);
    }

    /** Method menyimpan | menubah data */
    SaveAndNew(): void {
        const Data: JobTypeModel = this.FormInputData.value;

        if (this.inputFieldState == 'normal') {

            Data.id_job_type = 0;

            this.setupJobTypeService.onPostSave(Data)
                .subscribe((result) => {
            ...
            this.setupJobTypeService.onPutEdit(Data)
                .subscribe((result) => {
    ...
    DeleteData(): void {
        ...
        const Data: JobTypeModel = this.SelectedData;

        this.setupJobTypeService.onDelete(Data.id_job_type)
            .subscribe((result) => {
    ...
    get id_job_type(): AbstractControl { return this.FormInputData.get('id_job_type'); }
    get job_type(): AbstractControl { return this.FormInputData.get('job_type'); }
}
